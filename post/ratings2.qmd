...
This is the treatment you would use if you believed that substitution across pillars is possible but increasingly costly — and that zero on any pillar is catastrophic regardless of excellence elsewhere. Strict complements would be harsher still: $U = \min(S_1, S_2)$, where the score is determined entirely by the weakest pillar and additional investment in any stronger pillar is wasted. The geometric mean sits between the two extremes.

The choice between arithmetic and geometric mean is not a technical preference[^1]. Under the basic decency framing it is a substantive question: is a company that fails basic decency on one dimension rescued by excellence on another? The arithmetic mean says yes. The geometric mean says yes, but with increasing difficulty and with catastrophic consequences at zero. The Leontief minimum says no, absolutely — the weakest pillar is the only one that counts. The literature has not argued for one over the other because it has not recognised that the choice encodes a moral position.[^2]
...
![The flat plane (additive) allows Tesla (90,10) to score identically to (10,90). The curved bowl (multiplicative) collapses to zero along either axis — a zero on any pillar cannot be rescued by excellence elsewhere.](images/esg_surfaces_both.svg)
